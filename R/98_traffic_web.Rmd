...
traffic_8074 <- read_csv(here::here("data", "BCC", "traffic", "clean", "traffic_8074.csv"), col_types = cols(
  ...
  dplyr::select(-ds4, -rf4, -tsc, -ss, -married, 
...
traffic_8074 <- readRDS("../data/BCC/traffic/clean/traffic_8074.Rds") %>%  
  dplyr::filter(recorded >= ymd_hms("2018-10-08 00:00:00", tz = "Australia/Brisbane") & 
...
# fix vars order
traffic_8074 %<>% 
  dplyr::select(dbid, recorded, starts_with("recorded_"), everything())
```
...
```{r}
traffic_8074 %>% 
  dplyr::select(-dbid, -ct, -starts_with("recorded_")) %>% 
  slice(1:10)
```
...
traffic_8074_long %>% 
  dplyr::filter(recorded >= ymd_hms("2018-11-26 04:00:00", tz = "Australia/Brisbane") & 
...
traffic_8074_agg_long %>% 
  dplyr::filter(recorded_15 >= ymd_hms("2018-11-26 04:00:00", tz = "Australia/Brisbane") & 
...
```{r}
traffic_8074_agg_long %>% 
  dplyr::filter(recorded_15 >= ymd_hms("2018-10-08 00:00:00", tz = "Australia/Brisbane") & 
...
```{r}
traffic_8074_agg_long %>% 
  dplyr::filter(recorded_15 >= ymd_hms("2018-11-20 00:00:00", tz = "Australia/Brisbane") & 
