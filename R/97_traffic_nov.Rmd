...
intersect <- jsonlite::fromJSON(here::here("data", "BCC", "traffic", "raw", "locations",
                                ...
                                flatten = TRUE) %>% 
  dplyr::select(tsc, coordinates.latLng.latitude, coordinates.latLng.longitude,
         region, suburb) %>% 
  dplyr::filter(!is.na(coordinates.latLng.longitude) & !is.na(coordinates.latLng.latitude)) %>%
  dplyr::filter(tsc %in% c(8074, 878, 90)) %>%
...
bcc_traffic_8074 <- 
  read_table2("data/BCC/traffic_official/raw/B8074 15 Min Volumes November 2018.txt", 
              ...
              skip = 0) %>% 
  dplyr::rename(d1 = v3, d2 = v4, d3 = v5, 
         d4 = v6, d5 = v7, d6 = v8, 
         d7 = v9, d8 = v10, d9 = v11) %>% 
  dplyr::mutate(date = ifelse(test = v2 == "November", yes = paste0("2018-11-", v1), no = NA)) %>% 
  dplyr::select(-v1, -v2, -v12, -v13, - v14, - v15, -v16, -v17, -v18, -v19) %>% 
  tidyr::fill(date) %>% 
  dplyr::filter(time != "Approach") %>% 
  dplyr::filter(grepl(":",time)) %>% 
  dplyr::mutate(recorded_15 = anytime(paste0(date, " ", time, ":00"), tz = "Australia/Brisbane") - 15*60) %>% 
  dplyr::select(recorded_15, everything(), - date, -time) %>% 
  dplyr::mutate(d1 = as.integer(d1),  d2 = as.integer(d2), d3 = as.integer(d3),
