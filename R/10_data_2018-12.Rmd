---
title: "POP247 - data catalogue"
subtitle: "Pilot for St Lucia campus Oct 2018 - Jan 2019"
...
```{r eval=FALSE}
green_bridge_15M <- read_excel("data/counters/raw/420_2019-03-25-11-00-05.xlsx", 
...
saveRDS(green_bridge_15M, "data/counters/clean/green_bridge_15M.Rds")

# 1h aggregation
# green_bridge_15M$recorded_cat <- ymd_hms(cut(green_bridge_15M$date,
...
# 
# green_bridge_60M <- green_bridge_15M %>%
#   group_by(recorded_cat, type, site, direction) %>%
#   summarise(count = sum(count)) %>%
#   ungroup() %>%
#   rename(date = recorded_cat)
# 
# saveRDS(green_bridge_60M, "data/counters/clean/green_bridge_60M.Rds")
...
saveRDS(campus_stops, "data/goCard/clean/campus_stops.Rds")
```
...
```{r}
campus_gocard %>% 
  dplyr::select(-stop_id, -boarding_stop, -alighting_stop) %>% 
  slice(1:8)%>% 
  kable() %>%
...
saveRDS(campus_tweets, here::here("data", "Twitter", "clean", "campus_tweets.Rds"))
...
saveRDS(campus_places_times_long, 
        here::here("data", "google_places", "clean", "campus_places_times_long.Rds"))
