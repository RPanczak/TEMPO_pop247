...
Overview of two months, cyclists, outbound
...
Detailed example counts for week of `2018-12-03` across type and direction. Both original counts (`orig`) and counts corrected with occupancy are plotted.
...
Overview of two months, inbound
...
```{r echo=FALSE}
prime_agg <- readRDS("R:/WIFIUQHPC-Q0766/data/clean/processed_prime_reports/prime_agg_1.Rds") %>% 
...
uqweather <- readRDS(here::here("data", "uqweather", "clean", "uqweather.Rds")) %>% 
  ...
  filter(timestamp >= ymd_hms("2018-12-03 00:00:00", tz = "Australia/Brisbane") & 
           timestamp <= ymd_hms("2019-01-27 23:59:59", tz = "Australia/Brisbane")) %>% 
...
```{r eval=FALSE}
data %>% 
  filter(date >= ymd_hms("2019-01-06 23:59:59", tz = "Australia/Brisbane")) %>%
  dplyr::select(date, users_mac, ped_stock, veh_stock) %>% 
  gather(key = "type", value = "count", users_mac:veh_stock) %>% 
  ggplot(aes(x = date, y = count)) +
  geom_line() +
  theme(legend.position = "none") + 
  theme_minimal() + ylab("Connected users") +
...
M <- cor(dplyr::select(data, users_mac, 
                       ...
                       ped_net, ped_stock, veh_net, veh_stock), use = "complete.obs")
...
```{r echo=FALSE}
both2_fit <- bind_cols(time = data$time, date = data$date, count = data$users_mac,
                       fit = both2$summary.fitted.values$`0.5quant`,
                       lci = both2$summary.fitted.values$`0.025quant`, 
                       uci = both2$summary.fitted.values$`0.975quant`,
                       difference = round(data$users_mac - both2$summary.fitted.values$`0.5quant`),
                       difference_perc = round(((data$users_mac - both2$summary.fitted.values$`0.5quant`) / data$users_mac) *100, digits = 2)
)

ggplot(both2_fit, aes(x = time)) +
...
both2_fit %>% 
  ...
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = ymd_hms("2019-01-21 09:00:00", tz = "Australia/Brisbane"),
           xmax = ymd_hms("2019-01-21 17:00:00", tz = "Australia/Brisbane"),
           ymin = -Inf, ymax = Inf) +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = ymd_hms("2019-01-22 09:00:00", tz = "Australia/Brisbane"),
           xmax = ymd_hms("2019-01-22 17:00:00", tz = "Australia/Brisbane"),
           ymin = -Inf, ymax = Inf) +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = ymd_hms("2019-01-23 09:00:00", tz = "Australia/Brisbane"),
           xmax = ymd_hms("2019-01-23 17:00:00", tz = "Australia/Brisbane"),
           ymin = -Inf, ymax = Inf) +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = ymd_hms("2019-01-24 09:00:00", tz = "Australia/Brisbane"),
           xmax = ymd_hms("2019-01-24 17:00:00", tz = "Australia/Brisbane"),
           ymin = -Inf, ymax = Inf) +
  annotate("rect", fill = "grey", alpha = 0.4, 
           xmin = ymd_hms("2019-01-25 09:00:00", tz = "Australia/Brisbane"),
           xmax = ymd_hms("2019-01-25 17:00:00", tz = "Australia/Brisbane"),
           ymin = -Inf, ymax = Inf) +
...
Using data from two weeks of Jan (starting Mon 7th) we train the model and start predicting number of users in the nexttwo hours. Progressively when new data become available, they are included and new model is fitted with further projection horizon. 
...
for (i in start:end) {
  ...
  print(paste(Sys.time(), "Done time:", i))
}
...
ggplot(temp) +
  ...
  geom_text(data = filter(annotations, frame == frame_select), 
            aes(x = min(temp$date), 
                y = Inf, hjust = 0, vjust = 1, size = 10, label = text), 
            show.legend = FALSE) +
  scale_colour_manual(values = c("orange", "purple"),
                      name = "Data", labels = c("Counts", "Prediction (2h)")) + 
  guides(size = "none") +
  theme_minimal() + labs(title = 'Wi-Fi users on campus', x = '', y = 'Counts / prediction')
```
...
ggplot(temp) +
  ...
  scale_colour_manual(values = c("orange", "purple"),
                      name = "Data", labels = c("Counts", "Prediction (2h)")) + 
  guides(size = "none") +
  theme_minimal() + labs(title = 'Wi-Fi users on the campus', x = '', y = 'Counts / prediction')
```
...
```{r}
prediction_animate_08 <- ggplot(result) +
  geom_line(data = filter(data, date >= ymd_hms("2019-01-20 23:59:59", tz = "Australia/Brisbane")),
  ...
  geom_text(data = annotations,
            aes(x = ymd_hms("2019-01-21 00:00:00", tz = "Australia/Brisbane"),
                y = Inf, hjust = 0, vjust = 1, size = 10, label = text),
            show.legend = FALSE) +
  scale_colour_manual(values = c("orange", "purple"),
...
animate(prediction_animate_08, nframes = 2*max(result$frame), height = 600, width = 900)
anim_save("animations/wifi_prediction_animate_08.gif")
...
ggplot(annotations) +
  ...
  theme_minimal() + labs(title = 'Wi-Fi users on the campus', x = '', y = 'Mean counts - prediction (abs. %)')
```
